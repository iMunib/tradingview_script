import asyncio
import json
import websockets

async def main():
    browser_ws = "ws://127.0.0.1:9222/devtools/browser/f4211e06-b384-4568-9e42-fab8126bb7a9"
    target_id = "DB89DD8D0A3BA86237AB6883FC958F78"
    async with websockets.connect(browser_ws) as ws:
        await ws.send(json.dumps({
            "id": 1, "method": "Target.attachToTarget",
            "params": {"targetId": target_id, "flatten": True}
        }))
        sid = None
        while True:
            r = json.loads(await ws.recv())
            if r.get("id") == 1:
                sid = r["result"]["sessionId"]
                break
        
        js_code = """
        (() => {
            const bottomBar = document.querySelector('[data-name="footer-bottom-toolbar"]') || 
                              document.querySelector('#footer-chart-panel') ||
                              document.querySelector('[class*="bottom-widgetbar"]');
            const btns = Array.from(document.querySelectorAll('[class*="button-"], [data-name]'))
                .filter(b => b.parentElement && (b.parentElement.id.includes('footer') || b.closest('[class*="footer"]')))
                .map(b => ({
                    text: b.innerText ? b.innerText.trim() : '',
                    dataName: b.getAttribute('data-name') || '',
                    ariaLabel: b.getAttribute('aria-label') || ''
                })).filter(b => b.text || b.dataName || b.ariaLabel);
            return btns;
        })()
        """
        await ws.send(json.dumps({"id": 2, "sessionId": sid, "method": "Runtime.evaluate", "params": {"expression": js_code, "returnByValue": True}}))
        while True:
            r = json.loads(await ws.recv())
            if r.get("id") == 2:
                print("Footer buttons:", json.dumps(r["result"]["result"]["value"], indent=2))
                break

asyncio.run(main())
