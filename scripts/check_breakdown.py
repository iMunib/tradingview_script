...
async def main():
    ...
    async with websockets.connect(browser_ws) as ws:
        ...
        js_click = """
        (() => {
            const btn = Array.from(document.querySelectorAll('[role="tab"], button')).find(b => (b.innerText || '').trim() === 'Breakdown');
            if (btn) { btn.click(); return 'Clicked Breakdown'; }
            return 'Not found';
        })()
        """
        ...
        # Inspect table inside Breakdown
        js_table = """
        (() => {
            const report = document.querySelector('[class*="tableWrapper-"]') || document.querySelector('[class*="details-"]');
