...
async def main():
    ...
    async with websockets.connect(browser_ws) as ws:
        ...
        js_code = """
        (() => {
            // Find timeframe buttons or resolution selector
            const resButtons = Array.from(document.querySelectorAll('button, div[role="button"]'))
                .filter(el => ['1D', '1W', 'D', 'W', '1 day', '1 week'].includes((el.innerText || '').trim()))
                .map(el => ({
        ...
        while True:
            r = json.loads(await ws.recv())
            if r.get("id") == 2:
                print("Resolution buttons:", json.dumps(r["result"]["result"]["value"], indent=2))
                break
