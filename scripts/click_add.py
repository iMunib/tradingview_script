...
async def main():
    ...
    async with websockets.connect(browser_ws) as ws:
        ...
        js_code = """
        (() => {
            const btn = document.querySelector('[title="Add to chart"]') || document.querySelector('[title="Update on chart"]');
            if (btn) {
                btn.click();
                return 'Clicked: ' + (btn.getAttribute('title') || btn.innerText);
            }
            return 'Not found';
        })()
        """
        ...
        while True:
            r = json.loads(await ws.recv())
            if r.get("id") == 2:
                print("Click:", json.dumps(r["result"]["result"]["value"], indent=2))
                break
