...
async def main():
    ...
    async with websockets.connect(browser_ws) as ws:
        ...
        js_click = """
        (() => {
            const btn = Array.from(document.querySelectorAll('[role="tab"], button')).find(b => (b.innerText || '').trim() === 'Benchmarking');
            if (btn) { btn.click(); return 'Clicked Benchmarking'; }
            return 'Not found';
        })()
        """
        ...
        # Inspect table inside Benchmarking
        js_table = """
        (() => {
